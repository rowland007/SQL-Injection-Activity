/************************************************************************
 ...
 Date:          2021-11-07
 Description:   This assignment presents code that creates an in-memory database and runs a number of 
                queries with and without SQL injection, always dumping the results to the console. You 
                need to modify the run_query() function to detect a potential SQL injection attack, 
                prevent it, and display that information to the console. The specific type of SQL injection
                attack is the “OR value=value;” attack, and you must defend against that specific attack
                with value being any allowed SQL literal (numeric or quoted string).
 Input:         sqlite3.h
 Output:
 Known bugs:    SQL Injection in the run_query() fucntion
 Missing features:
 ...
 Modifications:
   Date                      Comment
 ---------   ------------------------------------------------
 2021NOV07   Add regex header and regular expression logic to run_query
 ************************************************************************/
...
static int callback(void *possible_vector, int argc, char **argv, char **azColName) {
    if (possible_vector == NULL) { // no vector passed in, so just display the results
...
bool run_query(sqlite3 *db, const std::string &sql, std::vector<user_record> &records) {
    ...
    // Using regular expressions to check for any input that is x=x or similar. If anything matches it returns false.
    std::regex regex("('(.*)[a-zA-Z](.*)'='(.*)[a-zA-Z](.*)')|([0-9]=[0-9])");
    std::smatch match;
    if (std::regex_search(sql.begin(), sql.end(), match, regex) && match.size() > 1) {
        std::cout << "SQL Injection attack detected - Terminating query" << std::endl;
        return false;
    }
...
bool run_query_injection(sqlite3 *db, const std::string &sql, std::vector<user_record> &records) {
    ...
    if (localCopy.find_last_of(str_where) >= 0) { // this sql has a where clause
        if (localCopy.back() == ';') { // smart SQL developer terminated with a semicolon - we can fix that!
...
void dump_results(const std::string &sql, const std::vector<user_record> &records) {
    ...
    for (auto record : records) {
        std::cout << "User: " << std::get<1>(record) << " [UID=" << std::get<0>(record) << " PWD="
